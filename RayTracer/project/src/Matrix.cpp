...
namespace dae {
	...
	{
		//todo W2
		//float yawInRadiands = yaw / 180 * PI;
		return(Matrix{ {cos(yaw),0,-sin(yaw),0},{0,1,0,0},{sin(yaw),0,cos(yaw),0},{0,0,0,1} }); //inverse
		//--
		throw std::runtime_error("Not Implemented Yet");
		return {};
	}
	...
	{
		//todo W2
		Matrix combined(CreateRotationX(r.x) * CreateRotationY(r.y) * CreateRotationZ(r.z));
		return combined;
